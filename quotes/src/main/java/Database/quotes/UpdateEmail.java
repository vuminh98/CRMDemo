...
public class UpdateEmail implements RequestHandler<String, String> {
		...
		try {
			...
			while (rs.next()) {
				...
				while (rSet.next()) {
					...
					ResultSet resultSet = statement2.executeQuery("UPDATE crm_email_list set "
							+ "EMAIL_LIST_NAME ='abc',EMAILS='abc@gmail.com',NOTE='asad' where account_id ='" + input + "'");	
					System.out.println(input);
